...
LIGAS = [
    ...
    {"liga": "Brasileirao Serie A", "id": 71, "temporada": 2026, "inicio": "2026-01-01"},
    ...
    {"liga": "Copa Chile", "id": 266, "temporada": 2026, "inicio": "2026-01-01"},
    ...
    {"liga": "Primera Division Bolivia", "id": 258, "temporada": 2026, "inicio": "2026-01-01"},
...
def api_get(endpoint, params=None):
    ...
    if response.status_code != 200:
        raise ValueError(f"Error API {response.status_code}: {response.text[:300]}")
    return response.json()


def obtener_partidos_liga(league_id, temporada, date_from, date_to):
...
def construir_fila(fixture, liga_nombre):
    ...
    if estado == "FT":
        try:
            ...
            for equipo_stats in stats:
                es_local = equipo_stats["team"]["name"] == local
    ...
    return {
        ...
        "tiros_total_local": tiros_total_local,
        "tiros_total_visitante": tiros_total_visitante,
    }
...
def descargar_y_guardar_csv(dias_adelante=4, descarga_inicial=False):
    ...
    temporada_default = hoy.year if hoy.month >= 7 else hoy.year - 1

    filas = []

    for comp in LIGAS:
        print(f"Descargando: {comp['liga']}...")
        temporada = comp["temporada"] if comp["temporada"] else temporada_default
        date_from = comp["inicio"] if descarga_inicial else (hoy - timedelta(days=7)).isoformat()

        try:
            partidos = obtener_partidos_liga(comp["id"], temporada, date_from, date_to)
            for fixture in partidos:
                estado = fixture.get("fixture", {}).get("status", {}).get("short", "")
                if estado in ("FT", "NS", "1H", "HT", "2H", "ET", "PEN", "AET", "SUSP"):
                    filas.append(construir_fila(fixture, comp["liga"]))
        except Exception as e:
            print(f"Error en {comp['liga']}: {e}")
            continue
    ...
    print(f"Partidos totales en CSV: {len(df_combined)}")
