...
resp = requests.get(
    ...
    headers=headers,
    params={"league": 130, "season": 2026, "from": "2026-05-01", "to": "2026-07-13"}
)
data = resp.json()
partidos = data.get("response", [])
print(f"Partidos encontrados en el hueco: {len(partidos)}")
...
for f in partidos:
    ...
    print(f"  OK: {home} vs {away} {f['goals']['home']}-{f['goals']['away']} ({estado})")
...
if filas:
    ...
    print(f"\nCSV actualizado: {len(filas)} partidos de Copa Argentina agregados/actualizados")
else:
