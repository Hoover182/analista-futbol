...
print(f"Re-descargando {len(mundial)} partidos con logica corregida...")
...
for idx, row in mundial.iterrows():
    ...
    for evento in eventos:
        ...
        # Excluir solo tanda de penales (minuto 120 con extra > 0)
        ...
        if tipo == "Goal":
            ...
            # TODOS los goles (Normal, Penalty, Own Goal) suman al equipo listado
            # La API lista Own Goals bajo el equipo que SE BENEFICIA
    ...
    if suma_local != goles_l_real or suma_visit != goles_v_real:
        errores += 1
        print(f"  DESCUADRE: {local} vs {visitante} | Real {goles_l_real}-{goles_v_real} | Calc {suma_local}-{suma_visit} (1T:{gl_1t}-{gv_1t} 2T:{gl_2t}-{gv_2t})")
        continue
...
print(f"\nOK: {corregidos} corregidos, {errores} con descuadre")
